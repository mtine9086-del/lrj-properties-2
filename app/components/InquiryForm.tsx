...
const BUDGET_PATTERN = /^[0-9A-Za-z₹,\.\-\s]+$/;
...
export default function InquiryForm() {
  ...
  const handleBudget = (value: string) => {
    const cleaned = value.replace(/[^0-9A-Za-z₹,.\-\s]/g, "").replace(/\s{2,}/g, " ").slice(0, 40);
    update("budget", cleaned);
  };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (location && !LOCATION_PATTERN.test(location)) {
      setStatus("error");
      setError("Please enter a valid preferred location.");
      return;
    }
    if (budget && !BUDGET_PATTERN.test(budget)) {
      setStatus("error");
      ...
      <form className={styles.form} onSubmit={submit} noValidate>
        ...
        <fieldset className={styles.consentBox}>
          ...
          <div className={styles.consentRow}>
            <input id="required-consent" name="consent_given" type="checkbox" checked={form.consent} onChange={(event) => update("consent", event.currentTarget.checked)} aria-required="true" />
            <label htmlFor="required-consent">
          ...
          <div className={styles.consentRow}>
            <input id="marketing-consent" name="marketing_consent" type="checkbox" checked={form.marketing_consent} onChange={(event) => update("marketing_consent", event.currentTarget.checked)} />
            <label htmlFor="marketing-consent">I separately consent to receive property updates, offers or promotional communications. This is optional and is not required to submit an enquiry.</label>
          </div>
        ...
        {status === "success" && <div className={styles.success} role="status"><CheckCircle2 size={19} /><div><strong>Inquiry submitted successfully.</strong><span>We have received your property request.</span></div></div>}
