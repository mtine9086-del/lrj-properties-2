  ...
  return (
    <div className="inquiryWrap">
      <div className="inquiryIntro">
        ...
        <div className="inquiryTrust">
          <span><CheckCircle2 size={15}/> Local property guidance</span>
          <span><CheckCircle2 size={15}/> Your details go securely through our API</span>
          <span><CheckCircle2 size={15}/> No Supabase credentials are exposed in the browser</span>
        </div>
      </div>
      ...
      <form className="inquiryForm" onSubmit={submit}>
        <div className="inquiryGrid">
          ...
          <label className="inquiryFull">Message<textarea value={form.message} onChange={(e) => update("message", e.target.value)} placeholder="Tell us what you need, preferred area, size, timeline, etc." rows={5} /></label>
        </div>

        {status === "success" && <div className="inquirySuccess"><CheckCircle2 size={19}/><div><strong>Inquiry submitted successfully.</strong><span>We have received your property request.</span></div></div>}
        {status === "error" && <div className="inquiryError">{error}</div>}

        <button className="primary inquirySubmit" type="submit" disabled={status === "loading"}>
          {status === "loading" ? <><Loader2 size={17} className="spin"/> Sending...</> : <><Send size={17}/> Send property inquiry</>}
        </button>
      </form>
    </div>
  );
