...
export default function HomePage() {
  const [open, setOpen] = useState(false); const [query, setQuery] = useState(''); const [slide, setSlide] = useState(0); const [cursor, setCursor] = useState({ x: 0, y: 0 });
  const filtered = useMemo(() => properties.filter(p => `${p.title} ${p.location} ${p.type}`.toLowerCase().includes(query.toLowerCase())), [query]);
  useEffect(() => { const move=(e:MouseEvent)=>setCursor({x:e.clientX,y:e.clientY}); window.addEventListener('mousemove',move); const t=window.setInterval(()=>setSlide(s=>(s+1)%heroSlides.length),6500); return()=>{window.removeEventListener('mousemove',move);window.clearInterval(t)}; },[]);
  useEffect(() => { const wheel=(e:WheelEvent)=>{if(Math.abs(e.deltaY)>35)setSlide(s=>(e.deltaY>0?(s+1)%heroSlides.length:(s-1+heroSlides.length)%heroSlides.length));}; window.addEventListener('wheel',wheel,{passive:true}); return()=>window.removeEventListener('wheel',wheel); },[]);
  ...
  return <main>
    ...
    <section id="top" className="hero heroSlider">
      <div className="heroBackdrop" key={active.image} style={{backgroundImage:`url(${active.image})`,transform:`translate3d(${cursor.x*.018-8}px,${cursor.y*.012-6}px,0) scale(1.08)`}}/><div className="heroShade"/><div className="heroGrid"/><div className="heroGlow"/>
      ...
      <div className="verticalPager">{heroSlides.map((_,i)=><button key={i} className={i===slide?'active':''} onClick={()=>setSlide(i)} aria-label={`Slide ${i+1}`}><span>0{i+1}</span></button>)}</div><div className="heroScroll"><ArrowDown size={15}/> SCROLL TO EXPLORE</div>
    </section>
